...
def build_model(y):
    ...
    model.add(Dense(2024, input_shape = (X_scaled.shape[1],), activation = 'relu'),
             Dropout(0.4))
...
################################# Cellular component model training #########################################################


Y_cc_train = np.load('final_data/Y_cc_train.npy')
...
x_develop, x_test, y_develop, y_test = train_test_split(X_scaled, Y_cc_train, test_size=0.2, random_state=seed)
...
final_model = build_model(Y_cc_train)
...
model_save_path = 'models/CC_model.h5'
...
cc_keep = np.load(f"metadata/cc_keep.npy", allow_pickle=True)
with open(f"metadata/go_cc_map.pkl", "rb") as f:
...
pred_out_csv = 'results/CC_predictions.csv'
...
submission_txt = ('metadata/cc_submission.txt')
