...
def build_model(y):
    ...
    model.add(Dense(1024, input_shape = (X_scaled.shape[1],), activation = 'relu'),
            Dropout(0.15))
...
################################# Biological process model training #########################################################


Y_bp_train = np.load('final_data/Y_bp_train.npy')
...
x_develop, x_test, y_develop, y_test = train_test_split(X_scaled, Y_bp_train, test_size=0.2, random_state=seed)
...
print(f"F-max Medio:     {np.mean(all_fmax):.4f} (+/- {np.std(all_fmax):.4f})")

'''


## Since the model has good validation parameters we can train the final model

final_model = build_model(Y_bp_train)
...
model_save_path = 'models/BP_model.h5'
...
bp_keep = np.load(f"metadata/bp_keep.npy", allow_pickle=True)
with open(f"metadata/go_bp_map.pkl", "rb") as f:
...
for i, pid in enumerate(test_ids):
    ...
    for j in idx:
        if int(j) not in inv_go_bp_map:
            continue
        go = inv_go_bp_map[int(j)]
        score = float(probs[j])
...
pred_out_csv = 'results/BP_predictions.csv'
...
submission_txt = ('metadata/bp_submission.txt')
